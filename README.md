...
Workspace numbers are rendered by the dock itself. Do not enable a second workspace widget, or the numbers will appear twice.
...
The widget renders numbered workspaces `1` through `5` and adds any additional numbered workspaces that Hyprland reports. Each workspace number is followed by its running application groups. Clicking a number switches to that workspace; clicking an application focuses, restores, or minimizes it. Install this widget in the left bar section and remove the separate `omarchy.workspaces` entry to avoid duplicate workspace numbers.

## Install

```bash
omarchy plugin add https://github.com/JoyfulOak/omarchy-workspace-dock.git --enable --yes
```

Placing the widget (defaults to the **left** bar section):

```bash
omarchy bar move io.github.joyfuloak.omarchy-workspace-dock --section left
# or: center / right
```
...
```bash
omarchy plugin enable io.github.joyfuloak.omarchy-workspace-dock
```

## Update

```bash
omarchy plugin update io.github.joyfuloak.omarchy-workspace-dock
```

## Remove

```bash
omarchy plugin remove io.github.joyfuloak.omarchy-workspace-dock --yes
```
...
```bash
omarchy bar set io.github.joyfuloak.omarchy-workspace-dock <key> <value>
```
...
Your pins are stored in `~/.local/state/omarchy/omarchy-workspace-dock/pinned.json`.
Pins from the former `~/.local/state/omarchy/top-bar-dock/pinned.json` location are migrated automatically.
