...
BarWidget {
  ...
  moduleName: "io.github.joyfuloak.omarchy-workspace-dock"
  ...
  readonly property var displayEntries: dock ? dock.displayEntries : []
  readonly property int currentWorkspaceId: dock ? dock.currentWorkspaceId : -1
  readonly property color activeWindowBorderColor: dock ? dock.activeBorderColor : Color.accent
  ...
  function workspaceKey(entry) {
    if (!entry || entry.kind === "pinned") return "pinned"
    var windows = entry.windows || (entry.toplevel ? [entry.toplevel] : [])
    for (var i = 0; i < windows.length; i++) {
      var workspace = windows[i] ? windows[i].workspace : null
      if (workspace) {
        var id = Number(workspace.id)
        if (isFinite(id) && id > 0) return String(id)
        var name = String(workspace.name || "")
        if (name && name.indexOf("special:") !== 0) return name
      }
    }
    return "unknown"
  }

  function workspaceDividerVisible(index) {
    if (index < 0 || index + 1 >= root.dockEntries.length) return false
    var current = root.workspaceKey(root.dockEntries[index])
    var next = root.workspaceKey(root.dockEntries[index + 1])
    return current !== next && next !== "pinned"
  }

  function workspaceDividerLabel(index) {
    if (index < 0 || index + 1 >= root.dockEntries.length) return ""
    var key = root.workspaceKey(root.dockEntries[index + 1])
    return key === "unknown" ? "Workspace" : "Workspace " + key
  }
  ...
  GridLayout {
    id: chipRow
    columns: root.vertical ? 1 : Math.max(1, root.displayEntries.length)
    ...
    Repeater {
      ...
      WidgetButton {
        ...
        readonly property bool workspaceSlot: modelData.kind === "workspace"
        readonly property bool separatorSlot: modelData.kind === "separator"
        readonly property var workspace: workspaceSlot ? root.workspaceById(modelData.workspaceId) : null
        readonly property bool workspaceOccupied: workspace !== null && workspace.toplevels.values.length > 0
        readonly property bool workspaceFocused: workspaceSlot && root.currentWorkspaceId === modelData.workspaceId
        readonly property bool separatorActive: separatorSlot && root.currentWorkspaceId === modelData.workspaceId
        ...
        readonly property string iconSource: pinnedSlot
          ? root.iconFromId(modelData.entryId, modelData.entry)
          : ((workspaceSlot || separatorSlot) ? "" : root.windowIcon(toplevel))
        ...
        readonly property bool minimized: !workspaceSlot && !pinnedSlot && windows.length > 0
          && windows.every(function(w) { return root.isMinimized(w) })
        readonly property bool focused: !workspaceSlot && !pinnedSlot && windows.length > 0
        ...
        readonly property bool draggedPin: !workspaceSlot && root.dragOrder !== null && chip.pinHost === root.dragId

        // Match the live Hyprland active-window border, refreshed after every
        // Hyprland config reload; no plugin-local color is hardcoded.
        bar: root.bar
        active: workspaceFocused
        activeColor: root.activeWindowBorderColor
        useActiveColor: true
        text: workspaceSlot ? (workspaceFocused ? "\uDB85\uDCFB" : String(modelData.workspaceId)) : ""
        // A separator is a real, reserved grid cell so it remains visible
        // between app groups instead of collapsing like an icon-less button.
        keepSpace: true
        hasVisualContent: !workspaceSlot
        labelVisible: workspaceSlot
        opacity: workspaceSlot ? (workspaceOccupied || workspaceFocused ? 1 : 0.5) : 1
        horizontalMargin: workspaceSlot ? 6 : 0
        verticalPadding: workspaceSlot ? 6 : 0
        fixedWidth: separatorSlot ? Style.space(8) : (workspaceSlot ? (root.vertical ? root.barSize : Style.space(20)) : root.barSize)
        fixedHeight: root.barSize
        clip: true
        dimmed: chip.draggedPin
        tooltipText: workspaceSlot
          ? "Workspace " + modelData.workspaceId
          : pinnedSlot
        ...
        onTooltipHoveredChanged: {
          if (tooltipHovered) {
            root.hoverChip = chip
            if (!pinnedSlot && !workspaceSlot) root.requestPreview(chip, modelData)
        ...
        onPressed: function(button) {
          ...
          if (button === Qt.LeftButton) {
            if (separatorSlot) {
              return
            } else if (workspaceSlot) {
              if (root.bar) root.bar.run("hyprctl dispatch " + Util.shellQuote("hl.dsp.focus({ workspace = \"" + modelData.workspaceId + "\" })"))
            } else if (pinnedSlot) {
          ...
          } else if (button === Qt.RightButton && !separatorSlot) {
        ...
        Rectangle {
          visible: chip.separatorSlot
          z: 10
          anchors.centerIn: parent
          width: root.vertical
            ? Math.max(Style.space(10), root.barSize - Style.space(6))
            : (chip.separatorActive ? Style.space(3) : Style.space(1))
          height: root.vertical ? Style.space(3) : Math.max(Style.space(10), root.barSize - Style.space(6))
          radius: Style.space(1)
          // Only the separator belonging to the active workspace uses the
          // active-window border color. Inactive groups match their number.
          color: chip.separatorActive
            ? root.activeWindowBorderColor
        ...
        Item {
          ...
          Image {
            id: appIcon
            anchors.fill: parent
            source: chip.workspaceSlot ? "" : chip.iconSource
          ...
          Text {
            anchors.centerIn: parent
            visible: !chip.workspaceSlot && !chip.separatorSlot && (appIcon.status === Image.Error || appIcon.source.toString() === "")
        ...
        Loader {
          id: segIndicator
          active: !chip.workspaceSlot && !chip.pinnedSlot && chip.windows && chip.windows.length > 0
  ...
  // Running entries are sorted by workspace. These dividers make each
  // workspace's group of app icons immediately visible without changing the
  // dock's hit targets or drag/reorder geometry.
  Repeater {
    model: 0

    Rectangle {
      required property int index
      readonly property bool dividerVisible: root.workspaceDividerVisible(index)
      readonly property string dividerLabel: root.workspaceDividerLabel(index)

      visible: dividerVisible
      z: 2
      x: root.vertical
        ? chipRow.x + (chipRow.width - width) / 2
        : chipRow.x + (index + 1) * root.barSize + index * Style.space(1) + (Style.space(1) - width) / 2
      y: root.vertical
        ? chipRow.y + (index + 1) * root.barSize + index * Style.space(1) + (Style.space(1) - height) / 2
        : chipRow.y + (chipRow.height - height) / 2
      width: root.vertical ? Math.max(Style.space(8), root.barSize - Style.space(8)) : Style.space(2)
      height: root.vertical ? Style.space(2) : Math.max(Style.space(8), root.barSize - Style.space(8))
      radius: Style.space(1)
      ...
      HoverHandler {
        onHoveredChanged: {
          if (!root.bar) return
          if (hovered) root.bar.showTooltip(parent, dividerLabel)
          else root.bar.hideTooltip(parent)
        }
      }

      Component.onDestruction: if (root.bar) root.bar.hideTooltip(this)
    }
  }
