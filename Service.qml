...
Item {
  ...
  readonly property string moduleName: "io.github.joyfuloak.omarchy-workspace-dock"
  ...
  readonly property string stateDir: stateHome + "/omarchy/omarchy-workspace-dock"
  readonly property string pinnedFilePath: stateDir + "/pinned.json"
  readonly property string legacyPinnedPath: stateHome + "/omarchy/top-bar-dock/pinned.json"
  ...
  // Bar plugins do not receive the host appLibrary facade. Keep a small local
  // icon index so desktop-entry icons are resolved the same way as the app menu.
  property var iconIndex: ({})
  property var pendingIconIndex: ({})
  // Match the live Hyprland active-window border, not the shell accent. The
  // fallback is used only until Hyprland returns its configured border color.
  property color activeBorderColor: Color.accent
  ...
  // Keep dockEntries app-only for existing pin/menu behavior; the widget uses
  // this combined rail to interleave workspace controls and running groups.
  readonly property var displayEntries: Dock.buildWorkspaceRail(dockEntries, workspaceIds)
  ...
  function iconFromEntry(entry) {
    ...
    var indexed = root.iconIndex[icon]
  ...
  function parseActiveBorderColor(raw) {
    // Hyprland serializes colors as AARRGGBB (for example fff313dd).
    // QML needs RRGGBB, so discard the leading alpha byte.
    var match = String(raw || "").match(/(?:0x)?([0-9A-Fa-f]{8}|[0-9A-Fa-f]{6})/)
    if (!match) return ""
    var hex = match[1]
    return "#" + (hex.length === 8 ? hex.slice(2) : hex)
  }

  function refreshActiveBorderColor() {
    if (!activeBorderProcess.running) activeBorderProcess.running = true
  }
  ...
  Connections {
    ...
    function onRawEvent(event) {
      ...
      if (name === "configreloaded") root.refreshActiveBorderColor()
  ...
  Process {
    id: activeBorderProcess
    command: ["/usr/bin/hyprctl", "-j", "getoption", "general:col.active_border"]
    stdout: StdioCollector { id: activeBorderOutput; waitForEnd: true }
    onExited: function(exitCode) {
      if (exitCode !== 0 || !activeBorderOutput.text) return
      try {
        var option = JSON.parse(activeBorderOutput.text)
        var parsed = root.parseActiveBorderColor(option.gradient)
        if (parsed) root.activeBorderColor = parsed
      } catch (err) {}
    }
  }
